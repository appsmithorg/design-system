...
| Component tokens | button-primary-bg-color, textarea-placeholder | These tokens apply to a specific UI component and hence are more relatable in order for one to reason how to style. | Offers full clarity in terms of which token to use, but is too specific and doesn’t scale when new components have to be introduced. Also, requires a token to be created for every component of the system and every time a new component is added to the system. These variables are not set on the root element,so they are scoped to the component they are defined in. |
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/b2954674-0901-488b-a570-3a83e04ae03a/Untitled.png)
...
- text color
- background color
- border (border color, stroke width etc)
- shadow (inset, outset shadows)

You will find tokens defined in the following format in Figma.
(Note: The exact name in Figma would be **colors/ui-control/regular/label.** This is due to how the tokens were defined in JSON for exporting to Token studio. We will correct this once we figure out how to do this well. In any case, if you search for a token — you will see it show up properly in Figma’s property pane.)
...
- Category tokens available for input controls:
    ...
    | State | Presentation attribute | Token name | token value |
    | --- | --- | --- | --- |
    | default | text | ui/control/label/text |  |
    |  | background-color | ui/control/default/bg-color |  |
    |  | border-color | ui/control/default/border-color |  |
    |  | border-style | ui/control/default/border-style |  |
    |  | shadow | ui/control/default/shadow |  |
    |  |  |  |  |
    | hover | text | ui/control/label/text | same as default |
    |  | background-color | ui/control/hover/bg-color |  |
    |  | border-color | ui/control/hover/border-color |  |
    |  | border-style | ui/control/hover/border-style |  |
    |  | shadow | ui/control/hover/shadow |  |
    |  |  |  |  |
    | active | text | ui/control/label/text | same as default |
    |  | background-color | ui/control/active/bg-color |  |
    |  | border-color | ui/control/active/border-color |  |
    |  | border-style | ui/control/active/border-style |  |
    |  | shadow | ui/control/active/shadow |  |
    |  |  |  |  |
    | :active:hover | text | ui/control/label/text | same as default |
    |  | background-color | ui/control/hover/bg-color |  |
    |  | border-color | ui/control/hover/border-color |  |
    |  | border-style | ui/control/hover/border-style |  |
    |  | shadow | ui/control/hover/shadow |  |
...
| --- | --- |
| label | text/ui/control/default/label |
| track | colors/ui/control/default/bg |
| knob | shadows/ui/control/default/surface |
...
| --- | --- |
| label | no change |
| track | colors/ui/control/active/bg |
| knob | shadows/ui/control/active/surface |
