...
Modal components are commonly used in user interfaces to display additional content or prompt for user interaction while blocking the main flow of the application. Use a modal to bring attention to a certain action that the user needs to take. In appsmith, modals will always have a title and may or may not be closed. The content of the modal can be customised. The action buttons in a modal are optional, but typically always present. When there is a group of action buttons, always place the primary button furthest to the right.
